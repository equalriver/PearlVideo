...
extension YPJOtherTool {
    ...
    ///   - dataStr: 二维码包含字符
    /// - Returns: 二维码图片
    public func createQRCode(size: CGFloat, dataStr: String) -> UIImage? {
        ...
        return createNonInterpolatedUIImage(image: cIImage, size: size)
    }
    ...
    /// - Returns: 自定义二维码
    public func createCustomizeQRCode(size: CGFloat,
                                     dataStr: String,
                                     imageType: QRCodeImageType,
                                     iconImage: UIImage,
                                     iconImageSize: CGFloat) -> UIImage? {
        guard let bgImage = createQRCode(size: size, dataStr: dataStr) else {
            return nil
        }
        var tempImage: UIImage? = iconImage
        if imageType == .CircularImage {
            tempImage = createCircularImage(image: iconImage)
        }
        
        var result :UIImage?
        if let t = tempImage {
            result = createNewImage(bgImage: bgImage, iconImage: t, iconSize: iconImageSize)
        }
        return result
    }
    ...
    ///   - QRImage: 二维码图片
    /// - Returns: 添加过背景的二维码
    public func addQRCodeBg(bgImage: UIImage, bgImageSize:CGFloat, QRImage: UIImage) -> UIImage? {
        let tempImage = imageCompress(sourceImage: bgImage, size: CGSize(width: bgImageSize, height: bgImageSize))
        return createNewImage(bgImage: tempImage!, iconImage: QRImage, iconSize: QRImage.size.width)
    }
    ...
    ///   - size: 图片大小
    /// - Returns: 图片
    private func createNonInterpolatedUIImage(image: CIImage, size: CGFloat) -> UIImage? {
    ...
    ///   - size: 显示到多大的区域
    /// - Returns: 处理好的图片
    private func imageCompress(sourceImage: UIImage, size:CGSize) -> UIImage? {
