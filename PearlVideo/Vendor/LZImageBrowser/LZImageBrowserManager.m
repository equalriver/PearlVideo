...
@interface LZImageBrowserManager ()<UIViewControllerPreviewingDelegate>
...
@end
@implementation LZImageBrowserManager

+ (LZImageBrowserManager *)imageBrowserManagerWithUrlStr:(NSArray<NSString *>*)imageUrls originImageViews:(NSArray<UIImageView *>*)originImageViews originController:(UIViewController *)controller forceTouch:(BOOL)forceTouchCapability forceTouchActionTitles:(nullable NSArray <NSString *>*)titles forceTouchActionComplete:(nullable ForceTouchActionBlock)forceTouchActionBlock {
    LZImageBrowserManager *imageBrowserManager = [[LZImageBrowserManager alloc] init];
    ...
    if (forceTouchCapability) {
        [imageBrowserManager initForceTouch];
    }
    if (forceTouchCapability && titles.count) {
        imageBrowserManager.previewActionTitls = titles;
        imageBrowserManager.forceTouchActionBlock = forceTouchActionBlock;
    }
    
    return imageBrowserManager;
}
